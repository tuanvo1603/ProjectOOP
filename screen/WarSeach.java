...
public class WarSeach extends JFrame {
	...
	public WarSeach() {
		...
		File folder = new File("src\\main\\resources\\Json\\Wars");
		File[] fileList = folder.listFiles();
		Gson gson = new Gson();

		ArrayList<War> warList = new ArrayList<>();
		for (File file : fileList) {
			try {
				String content = new String(Files.readAllBytes(file.toPath()));
				War war = gson.fromJson(content, War.class);
				warList.add(war);
			} catch (IOException e) {
		...
		for (War war : warList) {
			suggestionList.add(war.getName());
		}
		...
		table.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				if(e.getClickCount() == 1){
					...
					for (War war : warList) {
						if (war.getName().equalsIgnoreCase(selectedText)) {
							dispose();
