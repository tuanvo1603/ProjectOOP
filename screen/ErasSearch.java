...
public class ErasSearch extends JFrame {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					ErasSearch frame = new ErasSearch();
	...
	public ErasSearch() {
		...
		lblNewLabel_1.setBounds(-34, 0, 536, 490);
		getContentPane().add(lblNewLabel_1);

		JTextField txtNhpThngTi = new JTextField();
		txtNhpThngTi.setFont(new Font("Tahoma", Font.BOLD, 12));
		txtNhpThngTi.setSize(375, 27);
		txtNhpThngTi.setLocation(534, 34);
		txtNhpThngTi.setBorder(new CompoundBorder(new LineBorder(new Color(128, 128, 128), 2), new EmptyBorder(5, 5, 5, 5)));
		getContentPane().add(txtNhpThngTi);

		ArrayList<Era> peList = new ArrayList<>();

		File folder = new File("src\\main\\resources\\Json\\Eras");
		...
		txtNhpThngTi.addKeyListener(new java.awt.event.KeyAdapter() {
			public void keyReleased(java.awt.event.KeyEvent evt) {
				String text = txtNhpThngTi.getText().toLowerCase();
		...
		scrollPane.setBounds(501, 101, 471, 389);
		...
		table.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				if(e.getClickCount() == 1){
					...
					for (Era period : peList) {
						if (period.getEra().equalsIgnoreCase(selectedText)) {
							dispose();
							PeriodGui periodGui = new PeriodGui(period);
		...
		lblNewLabel_2.setIcon(new ImageIcon("src\\main\\resources\\Pictures\\gif (6).gif"));
		lblNewLabel_2.setBounds(500, 0, 471, 109);
		contentPane.add(lblNewLabel_2);

		JLabel lblNewLabel = new JLabel("");
		lblNewLabel.setIcon(new ImageIcon("src\\main\\resources\\Pictures\\giphy.jpg"));
		lblNewLabel.setBounds(0, 0, 502, 490);
